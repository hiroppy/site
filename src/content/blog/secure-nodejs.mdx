...
references:
  ...
  - https://nodejs.org/ja/learn/getting-started/security-best-practices
---

普段の開発ではパフォーマンスや機能追加に目が行きがちで、
セキュリティ対策は後回しにされることが多いのではないかと思います。
...
> **注意**: `--disable-proto=throw` は Node.js v18.19.0+ で `delete` モードが推奨され、
> **Node.js v22 では `throw` モードが削除されています**。
> 最新バージョンを使用している場合は `--disable-proto=delete` をご利用ください。
...
**Linux/macOS 専用**です。Windows では使用できません。
ヒープが枯渇すると、OpenSSL API がエラーを返すようになります。
...
Node.jsはデフォルトでHTTPヘッダーサイズを制限していますが、
**v14以降は16KB**、**v12以前は8KB**がデフォルト値です。
...
個人的には、少なくとも`--disable-proto=delete`は今後のプロジェクトで
