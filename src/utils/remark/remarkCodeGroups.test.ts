...
describe("remarkCodeGroups", () => {
  it("converts code-group fences into CodeGroup with panels and imports", () => {
    ...
    expect(getAttr(pnpmPanel, "class")).toBe("code-group-panel");
    expect(getAttr(pnpmPanel, "data-label")).toBe("pnpm");
    expect(getAttr(pnpmPanel, "data-icon")).toBe("mdi:language-typescript");
    ...
    expect(getAttr(npmPanel, "class")).toBe("code-group-panel");
    expect(getAttr(npmPanel, "data-label")).toBe("npm");
    expect(getAttr(npmPanel, "data-icon")).toBe("mdi:console");
  ...
  it("falls back to default labels when meta is missing", () => {
    ...
    expect(getAttr(panel, "class")).toBe("code-group-panel");
    expect(getAttr(panel, "data-label")).toBe("Tab 1");
    expect(getAttr(panel, "data-icon")).toBe("mdi:language-rust");
